...
<body>
		...
		<div class="row">
			<div class="col-lg-8 col-lg-offset-2">
				<div class="page-header">
					<h3>
						添加生理记录 <span style="color:red;">${result }</span>
					</h3>
				</div>
				<form id="addbodyForm" method="post" class="form-horizontal">
					...
					<div class="form-group" id="note">
						...
						<div class="col-lg-5">
							<input type="text" class="form-control" name="note" /><span id="tip">单位:cm</span>
						</div>
					</div>
					<div class="form-group">
	...
	<script type="text/javascript">
		$(function() {
			$('input[name=note]').attr('type', 'number');
			$("#choosetype").change(
					function() {
						$("#note").find("label").text(
								$(this).find("option:selected").text());
						if($(this).find("option:selected").text() == "身高"){
							$("#tip").text("单位:cm");
							$("input[name=note]").attr('type', 'number');
						}else if($(this).find("option:selected").text() == "体重"){
							$("#tip").text("单位:kg");
							$("input[name=note]").attr('type', 'number');
						}else{
							$("#tip").text("");
							$("input[name=note]").attr('type', 'text');
						}
